...
type WalletIllustrationProps = {
  className?: string;
  circleColors?: [string, string, string, string];
  currentStep: number;
};

type AnimatedEllipseProps = {
  cx: string;
  cy: string;
  rx: string;
  ry: string;
...
const AnimatedEllipse = memo(({ ...props }: AnimatedEllipseProps) => (
  <motion.ellipse {...props} />
));

const WalletIllustration = memo(
  ({
    className = "",
    circleColors = ["#5B48AD", "#DC1FFF", "#00FFA3", "#FECA1A"],
    currentStep,
  }: WalletIllustrationProps) => {
    const springTransition: Transition = Object.freeze({
      type: "spring",
      stiffness: 150,
      damping: 19,
      mass: 1.2,
    ...
    const getStepName = (step: number): "step1" | "step2" | "step3" => {
      if (step === 0) {
        return "step1";
      }
      if (step === 1) {
    ...
    const getBtcStepName = (
      step: number
    ): "btcStep0" | "btcStep1" | "hidden" => {
      if (step === 0) {
        return "btcStep0";
      }
      if (step === 1) {
    ...
    const getEthStepName = (
      step: number
    ): "ethStep0" | "ethStep1" | "hidden" => {
      if (step === 0) {
        return "ethStep0";
      }
      if (step === 1) {
    ...
    const getSolStepName = (
      step: number
    ): "solStep0" | "solStep1" | "hidden" => {
      if (step === 0) {
        return "solStep0";
      }
      if (step === 1) {
    ...
    return (
      <AnimatePresence>
        <div
          className={`relative mx-auto flex h-[200px] w-full max-w-[364px] justify-center ${className}`}
        >
          {currentStep === 0 && (
            <>
              <div className="absolute top-0 bottom-0 left-0 w-0">
                <motion.div
                  animate="visible"
                  className="absolute h-[50px] w-[50px] rounded-full opacity-70"
                  exit="hidden"
                  initial="hidden"
                  style={{
              ...
              <div className="absolute top-0 right-0 bottom-0 w-0">
                <motion.div
                  animate="visible"
                  className="absolute h-[50px] w-[50px] rounded-full opacity-70"
                  exit="hidden"
            ...
            <>
              ...
              >
                ...
                <path
                  ...
                  fill="#565656"
                />
                <circle cx="41" cy="35" fill="#E54033" r="15" />
                <circle cx="93" cy="35" fill="#FECA1A" r="15" />
                <circle cx="145" cy="35" fill="#AFD803" r="15" />
                <defs>
                  <linearGradient
              ...
              <motion.div
                animate="visible"
                className="absolute top-[48px] left-[62%] z-3 font-bold text-white text-xs"
                exit="hidden"
                initial="hidden"
          ...
          <motion.svg
            animate={getStepName(currentStep)}
            className="absolute top-[30%] z-1 h-auto w-[100px]"
            fill="none"
            height="286"
            initial="initial"
          ...
          <motion.svg
            animate={getBtcStepName(currentStep)}
            className="-translate-x-1/2 -translate-y-1/2 absolute top-1/2 left-1/2 z-2 h-[66px] w-[66px]"
            exit="hidden"
            fill="none"
            height="124"
            initial="hidden"
          ...
          <motion.svg
            animate={getEthStepName(currentStep)}
            className="-translate-x-1/2 -translate-y-1/2 absolute top-1/2 left-1/2 z-2 h-[66px] w-[66px]"
            exit="hidden"
            fill="none"
            height="100"
          ...
          >
            ...
            />
            <path
              d="M51.213 12.4189L27.9378 50.3521L51.213 39.9531V12.4189Z"
              fill="white"
            />
            <path
            ...
            />
            <path
              d="M51.2136 86.9029V68.1909L27.9384 54.6846L51.2136 86.9029Z"
              fill="white"
            />
            <path
              d="M51.2143 63.8653L74.4864 50.3528L51.2143 39.96V63.8653Z"
              fill="white"
              fillOpacity="0.2"
            />
            <path
          ...
          <motion.svg
            animate={getSolStepName(currentStep)}
            className="-translate-x-1/2 -translate-y-1/2 absolute top-1/2 left-1/2 z-2 h-[66px] w-[66px]"
            exit="hidden"
            fill="none"
            height="121"
          ...
          >
            <title>solana</title>
            <path
              d="M60.3375 120.675C93.7747 120.675 120.675 93.7748 120.675 61.667C120.675 27.6093 93.7747 0 60.3375 0C27.6093 0 -6.10352e-05 27.6093 -6.10352e-05 61.667C-6.10352e-05 95.7248 27.6093 123.334 60.3375 123.334Z"
              fill="currentColor"
            />
            <path
            ...
            <defs>
              ...
              >
                ...
                />
                <feGaussianBlur
                  result="effect1_foregroundBlur_23_285"
                  stdDeviation="22"
                />
              </filter>
              <linearGradient
...
const steps = [
  {
    ...
    content:
      "Wallets allow you to store, receive, send and interact with digital assets like NFTs and tokens in web applications.",
  },
  {
    title: "Security and Ease of Use",
    content:
      "With blockchain applications, your wallet is used as a secure and easy way to connect and interact with web applications.",
  },
  {
    title: "Decentralized and Permissionless",
    content:
      "An essential tool for using blockchain without permission. Wallets allow you to explore and participate in the new web.",
  },
];
...
function WalletOnboarding({ onClose }: WalletOnboardingProps) {
    ...
    <div className="flex w-full flex-col items-center">
      ...
      <div className="relative h-[120px] w-full">
        <AnimatePresence mode="popLayout">
          <motion.div
            ...
            className="absolute top-0 left-0 flex h-full w-full flex-col items-center justify-center text-center"
            exit={{ opacity: 0, scale: 0.6 }}
            initial={{ opacity: 0, scale: 0.6 }}
            key={currentStep}
            transition={{ duration: 0.3 }}
          >
            <h3 className="font-bold text-md">{steps[currentStep]?.title}</h3>
            <p className="font-normal text-muted-foreground text-sm leading-[21px]">
      ...
      {currentStep < steps.length - 1 && (
        <div className="flex w-full items-center justify-between gap-3">
          ...
          <Button className="flex-1" onClick={goToNext} variant="secondary">
            Next
          </Button>
        </div>
      )}
